...
std::vector<int> algorithms::iterateWindingPos(QPoint &q, std::vector<std::vector<QPoint>> pol_list)
{
    ...
    {
        if(getPositionWinding(q,pol_list[i])==1) res.push_back(i); //returns index of the first polygon that passes
    }
    return res;
}
...
std::vector<int> algorithms::iterateRayPos(QPoint &q, std::vector<std::vector<QPoint>> pol_list)
{
    ...
    {
        if(getPositionRay(q,pol_list[i])==1) res.push_back(i); //returns index of the first polygon that passes
    }
    return res;
}
